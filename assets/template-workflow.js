...
templateLibrary=function() {
 if(!state.templates.length) return state.isAdmin?'<div class="empty"><h3>No templates yet</h3><p>Upload a master PDF to begin.</p></div>':'<div class="empty"><h3>No approved templates yet</h3><p>Your administrators are still preparing the library.</p></div>';
 return `<div class="templates">${state.templates.map(t=>`<article class="template"><div class="preview"><span>${html(t.library_category || 'General')}</span><strong>${html(t.name)}</strong></div><div class="body"><div class="template-meta"><strong>${html(t.name)}</strong><span class="pill">${html(t.status)}</span></div><p class="muted">${html(t.description || '')}</p><small class="muted">${html(t.library_category || 'General')} · PDF</small>${state.isAdmin?`<div class="category-editor"><input data-template-category-input="${t.id}" list="templateCardCategoryOptions" maxlength="80" value="${html(t.library_category || 'General')}" aria-label="Category for ${html(t.name)}"><button type="button" class="btn outline" data-save-template-category="${t.id}">Save category</button></div>`:''}<div class="workflow-actions">${state.isAdmin?`<button class="btn outline" data-open-template="${t.id}">${t.status==='published'?'View Configuration':'Configure & Publish'}</button>`:''}${t.status==='published'?`<button class="btn" data-personalize-template="${t.id}">Personalize</button>${libraryToggleButton('template',t.id)}`:''}</div></div></article>`).join('')}</div>${state.isAdmin?categoryDatalist('templateCardCategoryOptions'):''}`;
};
