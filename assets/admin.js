...
function teamMemberRow(member) {
  ...
  const prefix = state.delegatedAccess ? 'Client ' : '';
  return `<div class="team-row"><div class="member-avatar" aria-hidden="true">${html(name.slice(0, 1).toUpperCase())}</div><div class="member-details"><strong>${html(name)}</strong><span>${html(email)}</span></div>${locked ? `<span class="pill">${member.role === 'owner' ? 'Owner' : `${prefix}${member.role}`}</span>` : `<div class="role-control"><select data-member-role="${member.user_id}" aria-label="Access level for ${html(name)}"><option value="member" ${member.role === 'member' || member.role === 'manager' ? 'selected' : ''}>${prefix}User</option><option value="admin" ${member.role === 'admin' ? 'selected' : ''}>${prefix}Admin</option></select><button type="button" class="btn outline" data-save-member-role="${member.user_id}">Save</button></div>`}</div>`;
}

function teamView() {
  const managedClient = Boolean(state.delegatedAccess);
  return `<div class="team-layout"><section class="card"><div class="eyebrow">${managedClient?'ADD A CLIENT EMPLOYEE':'ADD A TEAM MEMBER'}</div><h2>${managedClient?'Add an employee to':'Invite someone to'} ${html(state.organization.name)}</h2><p class="muted">${managedClient?'This person becomes a direct member of the client organization. Their files and activity remain owned by the client.':'They will receive a secure email invitation and join this organization after setting their password.'}</p><form id="teamInviteForm" class="settings-form"><label for="inviteName">Full name</label><input id="inviteName" name="fullName" type="text" maxlength="120" required><label for="inviteEmail">Work email</label><input id="inviteEmail" name="email" type="email" maxlength="255" required><label for="inviteRole">Access level</label><select id="inviteRole" name="role"><option value="member">${managedClient?'Client User':'User'} — use approved templates and files</option><option value="admin">${managedClient?'Client Admin':'Admin'} — manage templates, branding, and team</option></select><button class="btn" type="submit">${managedClient?'Add Client Employee':'Send invitation'}</button><div id="teamInviteMessage" class="form-message" role="status" aria-live="polite"></div></form></section><section class="card team-list-card"><div class="team-list-heading"><div><div class="eyebrow">${managedClient?'CLIENT ACCOUNT':'TEAM'}</div><h2>${managedClient?'Client employees':'Organization members'}</h2></div><span class="pill">${state.counts.members}</span></div><div id="teamMemberList">${organizationAdmin.loadingMembers ? '<p class="muted">Loading team…</p>' : organizationAdmin.members.map(teamMemberRow).join('') || `<p class="muted">${managedClient?'No client employees have been added yet.':'No team members found.'}</p>`}</div><div id="teamMessage" class="form-message" role="status" aria-live="polite"></div></section></div>`;
}
